<?php
App::uses('IagentsAppModel', 'Iagents.Model');
 ...
 * @property Post $Post
 */
class Aagent extends IagentsAppModel {
	public $actsAs = array(
				...
				if($output->status =='OK'){
					...
					foreach($output->results[0]->address_components as $key=>$value){
						...
						if(($value->types[0] =='administrative_area_level_1')|($value->types[0] =='postal_town')){
							$this->data['Aagent']['regionshort']= $value->short_name;
							$this->data['Aagent']['region'] = $value->long_name;
						}
						if($value->types[0] =='administrative_area_level_2'){
							$this->data['Aagent']['district'] = $value->short_name;
						}
						if($value->types[0] =='country'){
							$this->data['Aagent']['countryiso2'] = $value->short_name;
						}
						if($value->types[0] =='locality'){
							$this->data['Aagent']['city'] = $value->short_name;
						}
						$location[$value->types[0]] = $value->short_name;
					}			
					$this->data['Aagent']['formatedaddress']=$output->results[0]->formatted_address;
					$this->data['Aagent']['placeid']=$output->results[0]->place_id;
				}
				return true;
			}
		}
